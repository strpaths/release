...
class Pair {
	...
	public Pair(int l, int r, String left,
			String right) {
		this.l = l;
		this.r = r;
		this.left = left;
...
public class BiConnected {
	...
	Vector<Vector<Pair>> ans = new Vector<Vector<Pair>>();
	Vector<Pair> Glo = new Vector<Pair>();
	int n;
	...
	int dfs(int s, int t) {
		...
		for (int i = 0; i < e[s].size(); i++) {
			...
			if (dep[es] != 0) {
				...
				if (dep[es] < t)
					Glo.add(new Pair(s, es));
			} else {
				int temp = Glo.size();
				int a = dfs(es, t + 1);
				if (a >= t) {
					ans.add(new Vector<Pair>());
					while (Glo.size() > temp) {
	...
	Vector<Vector<Pair>> componentsOf(int s) {
		ans.clear();
		Glo.clear();
		for (int i = 0; i < MAX_N; i++) {
			dep[i] = 0;
	...
	Vector<HashSet<Integer>> edgeToVertex(Vector<Vector<Pair>> p)
	{
		Vector<HashSet<Integer>> v = new Vector<HashSet<Integer>>();
		HashSet<Integer> temp = new HashSet<Integer>();
		for(Vector<Pair> v1 : p)
		{
