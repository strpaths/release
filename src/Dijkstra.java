...
public final class Dijkstra {
    ...
    public <T> void shortestPaths(DestSrcSampleGraph graph, Integer source, boolean untilReachDest) {
         ...
         */
        for (int i = 0; i < graph.proteinsCount+1; i++)
        ...
        while (!pq.isEmpty()) {
            ...
            if(untilReachDest)
            {
	            for(int i = 0; i < graph.dests.length; i++)
	            	if(curr.getValue() == graph.dests[i])
	            	{
	            		dest = curr.getValue();
	            		return;
	            	}
    ...
    public ArrayList<Integer> getPath(Integer dest)
    {
